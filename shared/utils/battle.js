...
function calcTroopDmg(branchDef, tierData, troopDef, defMult, count, lvlMult, terrMult, atkMult, ignoreDef, isAtk, extraMult, round, troopBranch, armyAtkMult, armyFocMult) {
  if (!tierData) return 0;
  const dmgType = branchDef?.dmgType ?? "physical";
  const roll    = tierData.dmgLo + Math.random() * (tierData.dmgHi - tierData.dmgLo);
  // Magical bypasses physical DEF entirely; physical is resisted
  const resist  = dmgType === "magical" || ignoreDef
    ? 1.0
    : Math.max(0, 1 - (troopDef * defMult) / ((troopDef * defMult) + 80));
  const armyMult   = isAtk ? (dmgType === "magical" ? (armyFocMult||1) : (armyAtkMult||1)) : 1;
  const earlyMult  = dragonEarlyPenalty(troopBranch, round);
  const raw        = Math.max(1, Math.ceil(count)) * roll * lvlMult * terrMult * armyMult * (atkMult||1) * earlyMult * (extraMult||1);
    ...
    for (const ent of order) {
      ...
      // ── Attacker troops ───────────────────────────────────────────────────
        ...
        for (let hi = 0; hi < hits; hi++) {
          ...
          let dmg = calcTroopDmg(atkBranchDef, atkTierData, defTroopDef, defDown, count, atkLvlMult, 1, rs.troopAtkMult, false, true, vulnMult * mod, round, cmd.troopBranch, armyAtkMult, armyFocMult);
      ...
      // ── Defender commander ────────────────────────────────────────────────
        ...
        if (rs.enemyConfused > 0) {
          ...
          if (Math.random() < 0.5) {
            ...
            const red2     = Math.max(0, 1 - atkRes2/(atkRes2+80));
