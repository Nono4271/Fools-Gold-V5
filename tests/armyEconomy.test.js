...
test('late training waits safely when barracks cannot fit a command',()=>{let s=train(initial());const end=s.trainingQueues[0].commandMs*5;s={...s,troopCounts:{[key]:7450}};s=tick(s,end);assert.equal(s.trainingQueues[0].remaining,200);s={...s,troopCounts:{[key]:7300}};s=tick(s,end);assert.equal(s.troopCounts[key],7500);assert.equal(s.trainingQueues.length,0);});
test('pending training reserves capacity and poor players cannot pay',()=>{let s=train({...initial(),troopCounts:{[key]:7250}});assert.equal(train(s,{id:'second'}),s);const poor={...initial(),rss:{stone:0,wood:0,gas:0,food:0}};assert.equal(train(poor),poor);});
...
test('full barracks retain completed healing until room exists',()=>{let s=heal({...initial(),woundedByBranch:{[key]:10},troopCounts:{[key]:7500}},10);s=tick(s,10000);assert.equal(s.healQueue[0].remaining,10);s={...s,troopCounts:{[key]:7490}};s=tick(s,10000);assert.equal(s.troopCounts[key],7500);assert.equal(s.healQueue.length,0);});
...
test('barracks space is counted in commands: a medium command (50) takes as much room as a small one (100)',()=>{
 ...
 // Lv1 = 75 commands. 74 small commands (7400 troops) leaves exactly one command of room.
 const s0={...initial(),troopCounts:{[key]:7400}};
 ...
 // 75 commands of large units (300 troops) fill the barracks.
 const full={...initial(),troopCounts:{[bigKey]:CMD_SIZE.large*75}};
 assert.equal(train(full,{amount:100,unlocked}),full);
});
test('barracks holds 75 commands at Lv1 up to 1000 at Lv20',async()=>{
 const {barracksCommandCapacity}=await import('../shared/constants/buildings.js');
 assert.equal(barracksCommandCapacity(0),75);assert.equal(barracksCommandCapacity(1),75);assert.equal(barracksCommandCapacity(20),1000);
