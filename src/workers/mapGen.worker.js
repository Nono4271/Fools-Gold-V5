...

{
  ...
  // ── P10–P13 road overlap cleanup ─────────────────────────────────────────────
  // Any 2×2 structure whose cells overlap a road gets relocated within 50 tiles
  // in the same region. If no valid slot found, demote to P9.
  {
    const SEARCH_RADIUS = 50;
    const BLOCKED_FLAGS = F_KEEP|F_KEEPPART|F_HQ|F_HQPART|F_GATE|F_BORDER;
    let overlapCount = 0, relocatedCount = 0, demotedCount = 0;

    for (let r2 = 0; r2 < ROWS - 1; r2++) {
      for (let c2 = 0; c2 < COLS - 1; c2++) {
        const idx2 = r2 * COLS + c2;
        if (!(flagArr[idx2] & F_KEEP)) continue;
        const pl2 = powerArr[idx2];
        if (pl2 < 10) continue;

        // Check if any of the 4 cells overlap a road
        const cells = [[c2,r2],[c2+1,r2],[c2,r2+1],[c2+1,r2+1]];
        const onRoad = cells.some(([tc,tr]) => {
          const t = terrainArr[tr*COLS+tc];
          return t === TERRAIN_ENC.road || t === TERRAIN_ENC.hellfire;
        });
        if (!onRoad) continue;
        overlapCount++;

        // Erase current 2×2 footprint
        const srcRegion = regionArr[idx2];
        for (const [tc, tr] of cells) {
          const ti = tr * COLS + tc;
          flagArr[ti]     = flagArr[ti] & ~(F_KEEP|F_KEEPPART);
          powerArr[ti]    = 9;
          garrisonArr[ti] = 0;
          siegeArr[ti]    = 0;
          siegeMaxArr[ti] = 0;
        ...
        delete keepMeta[`${c2},${r2}`];

        // Spiral outward to find a valid replacement slot
        let placed = false;
        outer:
        for (let dist = 1; dist <= SEARCH_RADIUS; dist++) {
          for (let dr = -dist; dr <= dist; dr++) {
            for (let dc = -dist; dc <= dist; dc++) {
              if (Math.abs(dr) !== dist && Math.abs(dc) !== dist) continue; // shell only
              const nc = c2 + dc, nr = r2 + dr;
              if (nc < 0 || nr < 0 || nc >= COLS - 1 || nr >= ROWS - 1) continue;
              const ncells = [[nc,nr],[nc+1,nr],[nc,nr+1],[nc+1,nr+1]];
              let blocked = false;
              for (const [tc, tr] of ncells) {
                const ti = tr * COLS + tc;
                if (regionArr[ti] !== srcRegion) { blocked = true; break; }
                if (flagArr[ti] & BLOCKED_FLAGS) { blocked = true; break; }
                if (KEEP_FOOTPRINT_SET.has(`${tc},${tr}`)) { blocked = true; break; }
                const t = terrainArr[ti];
                if (t === TERRAIN_ENC.road || t === TERRAIN_ENC.hellfire) { blocked = true; break; }
              }
              if (blocked) continue;
              ...
              const siege2 = P10_SIEGE[pl2] ?? 8000;
              const primaryRss = rssArr[nidx];
              flagArr[nidx]     = (flagArr[nidx] & ~(F_KEEPPART|F_HQ|F_HQPART)) | F_KEEP;
              powerArr[nidx]    = pl2;
              regionArr[nidx]   = srcRegion;
              garrisonArr[nidx] = Math.round(POWER_DEFS[pl2].command * 100);
              siegeArr[nidx]    = siege2;
              siegeMaxArr[nidx] = siege2;
              for (const [tc, tr] of [[nc+1,nr],[nc,nr+1],[nc+1,nr+1]]) {
                const ti = tr * COLS + tc;
                powerArr[ti]    = pl2;
                regionArr[ti]   = srcRegion;
                flagArr[ti]     = (flagArr[ti] & ~(F_KEEP|F_HQ|F_HQPART|F_WIN)) | F_KEEPPART;
                keepPrimArr[ti] = nidx;
                rssArr[ti]      = primaryRss;
              }
        ...
        // If no slot found within radius, tile already demoted to P9 above
        if (!placed) demotedCount++;
      }
    }
    console.log(`[MapGen] P10+ road cleanup: ${overlapCount} overlaps found, ${relocatedCount} relocated, ${demotedCount} demoted to P9 (${overlapCount > 0 ? Math.round(relocatedCount/overlapCount*100) : 100}% saved)`);
  }
