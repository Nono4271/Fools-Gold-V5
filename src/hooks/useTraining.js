...
export function useTraining({ screen, bldgs, setTrainingQueues, setTroopCounts, setBarracks, setWounded, woundedQueue, setWoundedQueue, trainingSpeedMult = 1, healQueue, setHealQueue, healSpeedMult = 1 }) {
  ...
  // ── Heal queue tick — 40% of train rate ──────────────────────────────────
  // healQueue: [{ id, branchKey, remaining, total }]
  // Delivers healed troops directly to barracks pool (not troopCounts —
  // healed troops go back to the general pool, not per-branch).
  useEffect(() => {
    if (screen !== "game") return;
    const id = setInterval(() => {
      if (!healQueue || healQueue.length === 0) return;
      const b        = bldgsRef.current;
      const rate     = Math.max(1, Math.floor(trainRate(b.training || 0) * 0.40 * (healSpeedMult || 1)));
      const perQueue = Math.max(1, Math.floor(rate / healQueue.length));
      const cap      = barracksCapacity(b.barracks || 0);

      setHealQueue(queues => {
        if (!queues || !queues.length) return queues;
        let totalHealed = 0;
        const updated = queues.map(q => {
          const delivered    = Math.min(q.remaining, perQueue);
          totalHealed += delivered;
          const newRemaining = q.remaining - delivered;
          return newRemaining <= 0 ? null : { ...q, remaining: newRemaining };
        }).filter(Boolean);

        if (totalHealed > 0) {
          setBarracks(pool => Math.min(cap, pool + totalHealed));
          setTroopCounts(counts => {
            // Return healed troops to their original branch pools
            const next = { ...counts };
            // Distribute proportionally by what each queue contributed
            let remaining = totalHealed;
            for (const q of queues) {
              if (remaining <= 0) break;
              const delivered = Math.min(q.remaining, perQueue);
              if (delivered > 0 && q.branchKey) {
                next[q.branchKey] = (next[q.branchKey] || 0) + delivered;
                remaining -= delivered;
              }
            }
            return next;
    ...
    return () => clearInterval(id);
  }, [screen, healQueue, setHealQueue, setBarracks, setTroopCounts, healSpeedMult]);
